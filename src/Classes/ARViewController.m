...
- (id)init {
	...
	ar_shownCoordinates = [[NSMutableArray alloc] init];
	...
	_currentDegree = 0.0;
	compassNeedle = [[UIImageView alloc] initWithFrame:CGRectMake(5.0f, 5.0f, 75.0f, 75.0f)];
	compassNeedle.image = [UIImage imageNamed:@"CompassNeedle.png"];
...
- (void)loadView {
	...
	//ar_overlayView.backgroundColor = [UIColor colorWithRed:35.0 / 255.0 green:89.0 / 255.0 blue:172.0 / 255.0 alpha:1.0];
	
	UIImageView *compass = [[UIImageView alloc] initWithFrame:CGRectMake(5.0f, 5.0f, 75.0f, 75.0f)];
	compass.image = [UIImage imageNamed:@"Compass.png"];
	
	[ar_overlayView addSubview:compass];
	[compass release];

	[ar_overlayView addSubview:compassNeedle];
...
- (CGPoint)pointInView:(UIView *)realityView forCoordinate:(ARCoordinate *)coordinate {
	...
	//NSLog(@"leftAzimuth: %f", leftAzimuth);
...
- (void)updateLocations:(NSTimer *)timer {
	...
	NSString *FIRST = @"CoordinateID";
	NSString *LAST = @"RadialDistance";
	...
	for (ARCoordinate *item in ar_coordinates) {
		...
		if ([self viewportContainsCoordinate:item]) {
			...
			viewToDraw.layer.anchorPoint = CGPointMake(0.5, 1.0);
						
			float width = 150.0;
			float height = 100.0;
			
			//((UILabel *)[viewToDraw viewWithTag:999]).text = [NSString stringWithFormat:@"%@ - %5.2fkm", item.title, item.radialDistance / 1000.0];
			((UILabel *)[viewToDraw viewWithTag:999]).text = item.description;
			
			//viewToDraw.titleLabel.text = [NSString stringWithFormat:@"%@ - %5.2fkm", item.title, item.radialDistance / 1000.0];
			
			//viewToDraw.center = CGPointMake(150 / 2, 100);
			
			float posX = loc.x - width / 2;
			float posY = loc.y - height;
			
			//viewToDraw.frame = CGRectMake(loc.x - width / 2.0, loc.y - height, width, height);
			viewToDraw.frame = CGRectMake(posX, posY, width, height);
			...
			if (!(viewToDraw.superview)) {
				...
				NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:
											[NSNumber numberWithInt:item.coordinateID], FIRST, [NSNumber numberWithDouble:item.radialDistance], LAST, nil];
				
				[ar_shownCoordinates addObject:dict];
				
				[ar_overlayView addSubview:viewToDraw];
				
				NSSortDescriptor *firstDescriptor = [[[NSSortDescriptor alloc] initWithKey:LAST ascending:YES selector:nil] autorelease];
				NSArray *sortedArray = [ar_shownCoordinates sortedArrayUsingDescriptors:[NSArray arrayWithObjects:firstDescriptor, nil]];

				for(NSDictionary *element in sortedArray) {
					[ar_overlayView sendSubviewToBack:[ar_overlayView viewWithTag:[((NSNumber *)[element objectForKey:FIRST]) intValue]]];
				}
			}
		} else {
			NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:
			...
			[ar_shownCoordinates removeObject:dict];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	_currentDegree += (_startPoint.x - [touch locationInView:self.view].x) / 10.0;
	
	if (_currentDegree > 360) {
		_currentDegree = 0 + (_currentDegree - 360);
	}
	
	if (_currentDegree < 0) {
		_currentDegree = 360 - _currentDegree;
	}
	

	//compassNeedle.transform = CGAffineTransformRotate(compassNeedle.transform, degreesToRadians(_currentDegree));
	compassNeedle.transform = CGAffineTransformMakeRotation(degreesToRadians(_currentDegree));
	//self.centerCoordinate.azimuth = fmod(_currentDegree, 360.0) * (M_PI / 180.0);
	self.centerCoordinate.azimuth = degreesToRadians(_currentDegree);
	//NSLog(@"%f - %f", fmod(_currentDegree, 360.0), degreesToRadians(_currentDegree));
	_startPoint = [touch locationInView:self.view];
}

- (void)locationManager:(CLLocationManager *)manager didUpdateHeading:(CLHeading *)newHeading {
	
	compassNeedle.transform = CGAffineTransformMakeRotation(newHeading.magneticHeading);
	//self.centerCoordinate.azimuth = fmod(newHeading.magneticHeading, 360.0) * (M_PI / 180.0);	
	self.centerCoordinate.azimuth = degreesToRadians(newHeading.magneticHeading);
...
- (void)dealloc {
	[ar_debugView release];
	[compassNeedle release];
	
	[ar_coordinateViews release];
	[ar_coordinates release];
	[ar_shownCoordinates release];
